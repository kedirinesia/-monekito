...
class _TransactionFormPageState extends State<TransactionFormPage>
  ...
  submitForm() {
    debugPrint('[TransactionForm] Submit form called');
    debugPrint('[TransactionForm] Transaction type: ' + transactionType.toString());
    debugPrint('[TransactionForm] Transaction value: ' + transactionValue.toString());
    debugPrint('[TransactionForm] From account: ' + (fromAccount?.name ?? 'null'));
    debugPrint('[TransactionForm] Selected category: ' + (selectedCategory?.name ?? 'null'));
    
    if (transactionType.isIncomeOrExpense && selectedCategory == null ||
        transactionType.isTransfer && transferAccount == null) {
      debugPrint('[TransactionForm] Validation failed: missing category or transfer account');
    ...
    // Validate balance for expense transactions and outgoing transfers
    if ((transactionType == TransactionType.E || transactionType == TransactionType.T) && fromAccount != null) {
      _validateBalanceAndProceed();
      return;
    }
    ...
    postCall.then((value) async {
      debugPrint('[TransactionForm] Transaction saved successfully with ID: ' + value.toString());
      ...
      try {
        debugPrint('[TransactionForm] Processing tags - removing: ' + tagsToRemove.length.toString() + ', adding: ' + tagsToAdd.length.toString());
        // Remove tags
        ...
        debugPrint('[TransactionForm] Tags processed successfully, calling onSuccess');
        
        String transactionTypeText = '';
        switch (transactionType) {
          case TransactionType.E:
            transactionTypeText = 'Pengeluaran';
            break;
          case TransactionType.I:
            transactionTypeText = 'Pendapatan';
            break;
        ...
        NotificationService().showTransactionSuccessNotification(
          transactionType: transactionTypeText,
          amount: transactionValue.abs(),
        ...
        debugPrint('[TransactionForm] Error processing tags: ' + error.toString());
        Navigator.pop(context);
        
        NotificationService().showErrorNotification(
          title: 'Error',
          body: 'Gagal memproses tags: ${error.toString()}',
        );
      }
    }).catchError((error) {
      debugPrint('[TransactionForm] Error saving transaction: ' + error.toString());
      
      NotificationService().showErrorNotification(
        title: 'Error',
        body: 'Gagal menyimpan transaksi: ${error.toString()}',
      );
    });
  }
      ...
      persistentFooterButtons: [
        PersistentFooterButton(
          child: FilledButton.icon(
            onPressed: () {
              debugPrint('[TransactionForm] Save button pressed at: ' + DateTime.now().toIso8601String());
              if (_formKey.currentState!.validate()) {
                debugPrint('[TransactionForm] Form validation passed, saving...');
                _formKey.currentState!.save();

                submitForm();
              } else {
                debugPrint('[TransactionForm] Form validation failed');
                NotificationService().showErrorNotification(
                  title: 'Validasi Gagal',
                  body: 'Mohon lengkapi semua field yang diperlukan',
                );
              }
            },
  ...
  void _validateBalanceAndProceed() async {
    ...
    try {
      ...
      if (isValid) {
        // If balance is sufficient, proceed with transaction creation
        _proceedWithTransaction();
      }
      // If not valid, error message is already shown by the validator
    } catch (error) {
      debugPrint('[TransactionForm] Error validating balance: $error');
      scMessenger.showSnackBar(
        SnackBar(
  ...
  void _proceedWithTransaction() {
    ...
    postCall.then((value) async {
      ...
      try {
        // Remove tags that are no longer associated with the transaction
        for (final tag in tagsToRemove) {
          await (db.delete(db.transactionTags)
                ..where((tbl) =>
                    tbl.transactionID.equals(newTrID) & tbl.tagID.equals(tag.id)))
              .go();
        }
        ...
        debugPrint('[TransactionForm] Error processing tags: $error');
        NotificationService().showErrorNotification(
          title: 'Error',
          body: 'Gagal memproses tags: ${error.toString()}',
        );
